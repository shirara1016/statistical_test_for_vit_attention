...
from source.new_grid import (
    make_image,
    create_cov_matrix,
    DeterministicGridBasedSIforViT,
)
...
class ExperimentViT(PararellExperiment):
    ...
    def iter_experiment(self, args) -> tuple:
        ...
        si = DeterministicGridBasedSIforViT(
            self.d,
            vit_visualizer,
            cov=self.cov,
            mode=self.mode,
...
if __name__ == "__main__":
    ...
    file_name = (
        f"{args.mode}_seed{args.seed}d{args.d}signal{args.signal}{architecture}.pkl"
    )
    print(file_name)
    print()
